...
# For static images:
IMAGE_FILES = ["female5.jpg"]
drawing_spec = mp_drawing.DrawingSpec(thickness=1, circle_radius=1)
sunglasses = cv2.imread('sunglasses_bg_transparent.png', cv2.IMREAD_UNCHANGED)

# Lip connections (FACEMESH_LIPS)
...
face_landmark_numbers = set()
for pair in FACEMESH_TESSELATION:
    face_landmark_numbers.add(pair[0])
    face_landmark_numbers.add(pair[1])

# ランドマークの番号をセットに入れる
lips_landmark_numbers = set()
for pair in FACEMESH_LIPS:
    lips_landmark_numbers.add(pair[0])
    lips_landmark_numbers.add(pair[1])

left_eye_landmark_numbers = set()
for pair in FACEMESH_LEFT_EYE:
    left_eye_landmark_numbers.add(pair[0])
    left_eye_landmark_numbers.add(pair[1])

right_eye_landmark_numbers = set()
for pair in FACEMESH_RIGHT_EYE:
    right_eye_landmark_numbers.add(pair[0])
    right_eye_landmark_numbers.add(pair[1])

nose_landmark_numbers = set()
for pair in FACEMESH_NOSE:
    nose_landmark_numbers.add(pair[0])
    nose_landmark_numbers.add(pair[1])

left_eye_brow_landmark_numbers = set()
for pair in FACEMESH_LEFT_EYEBROW:
    left_eye_brow_landmark_numbers.add(pair[0])
    left_eye_brow_landmark_numbers.add(pair[1])

right_eye_brow_landmark_numbers = set()
for pair in FACEMESH_RIGHT_EYEBROW:
    right_eye_brow_landmark_numbers.add(pair[0])
    right_eye_brow_landmark_numbers.add(pair[1])

face_oval_landmark_numbers = set()
for pair in FACEMESH_FACE_OVAL:
    face_oval_landmark_numbers.add(pair[0])
    face_oval_landmark_numbers.add(pair[1])


left_eye_upper_line = [362, 398, 384, 385, 386, 387, 388, 466, 263]
right_eye_upper_line = [33, 246, 161, 160, 159, 158, 157, 173, 133]

left_eye_shadow_upper_line = [362, 414, 286, 258, 257, 259, 260, 467, 263]
left_eye_shadow = np.concatenate((left_eye_upper_line, left_eye_shadow_upper_line[::-1]))

right_eye_shadow_upper_line = [33, 130, 247, 30, 29, 27, 28, 56, 190, 243, 133]
right_eye_shadow = np.concatenate((right_eye_upper_line, right_eye_shadow_upper_line[::-1]))

# lip_outer = [0, 17, 37, 39, 40, 61, 84, 91, 146, 181, 185, 267, 269, 270, 291, 314, 321, 375, 405, 409]
...
lip_outer = np.concatenate((lip_upper_outer, lip_upper_inner[::-1]))
lip_inner = np.concatenate((lip_lower_outer, lip_lower_inner[::-1]))

filtered_face_numbers = (
    face_landmark_numbers
    - lips_landmark_numbers
    - left_eye_landmark_numbers
    - right_eye_landmark_numbers
    - nose_landmark_numbers
    - left_eye_brow_landmark_numbers
    - right_eye_brow_landmark_numbers
    - face_oval_landmark_numbers
)

sorted_filtered_face_numbers = sorted(filtered_face_numbers)
...
eye_shadow_color = purple
eye_line_color = black
rip_color = crimson
eye_line_thickness = 3
ksize = (51, 51)
# # Webカメラからの入力を設定
...
with mp_face_mesh.FaceMesh(
    static_image_mode=True,  # False 動画モード
    max_num_faces=1,
    ...
    for idx, file in enumerate(IMAGE_FILES):
        ...
        #     .get_default_face_mesh_iris_connections_style())
            ...
            #####リップ上下#####
            lip_outer_points = []
            for num in lip_outer:
                x = int(face_landmarks.landmark[num].x * image.shape[1])
                y = int(face_landmarks.landmark[num].y * image.shape[0])
                lip_outer_points.append((x, y))

            lip_inner_points = []
            for num in lip_inner:
                x = int(face_landmarks.landmark[num].x * image.shape[1])
                y = int(face_landmarks.landmark[num].y * image.shape[0])
                lip_inner_points.append((x, y))
                
            # 左目のアイシャドウ領域のポイントを計算
            left_eye_shadow_points = []
            for i, num in enumerate(left_eye_shadow):
                x = int(face_landmarks.landmark[num].x * image.shape[1])
                y = int(face_landmarks.landmark[num].y * image.shape[0])
                left_eye_shadow_points.append((x, y))
            left_eye_shadow_points = np.array(left_eye_shadow_points, dtype=np.int32)

            # 左目のマスク作成
            left_eye_shadow_mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8)
            cv2.fillPoly(left_eye_shadow_mask, [left_eye_shadow_points], 255)

            # 左目のマスクのグラデーション生成
            left_eye_shadow_distance_mask = cv2.distanceTransform(left_eye_shadow_mask, cv2.DIST_L2, 5)
            left_eye_shadow_distance_mask = cv2.normalize(left_eye_shadow_distance_mask, None, 0.0, 1.0, cv2.NORM_MINMAX)

            # 左目のマスクをアルファ値に変換
            left_eye_shadow_alpha = np.clip(left_eye_shadow_distance_mask, 0, 1)

            # 左目のアイシャドウカラーを適用
            left_eye_shadow_colored_fill = np.zeros_like(image, dtype=np.uint8)
            cv2.fillPoly(left_eye_shadow_colored_fill, [left_eye_shadow_points], eye_shadow_color)

            # 左目のアイシャドウを元画像とブレンド
            left_annotated_image = (image.astype(np.float32) * (1 - left_eye_shadow_alpha[:, :, None]) + 
                                    left_eye_shadow_colored_fill.astype(np.float32) * left_eye_shadow_alpha[:, :, None]).astype(np.uint8)

            ##### 右目のアイシャドウ #####
            right_eye_shadow_points = []
            for i, num in enumerate(right_eye_shadow):
                x = int(face_landmarks.landmark[num].x * image.shape[1])
                y = int(face_landmarks.landmark[num].y * image.shape[0])
                right_eye_shadow_points.append((x, y))
            right_eye_shadow_points = np.array(right_eye_shadow_points, dtype=np.int32)

            # 右目のマスク作成
            right_eye_shadow_mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8)
            cv2.fillPoly(right_eye_shadow_mask, [right_eye_shadow_points], 255)

            # 右目のマスクのグラデーション生成
            right_eye_shadow_distance_mask = cv2.distanceTransform(right_eye_shadow_mask, cv2.DIST_L2, 5)
            right_eye_shadow_distance_mask = cv2.normalize(right_eye_shadow_distance_mask, None, 0.0, 1.0, cv2.NORM_MINMAX)

            # 右目のマスクをアルファ値に変換
            right_eye_shadow_alpha = np.clip(right_eye_shadow_distance_mask, 0, 1)

            # 右目のアイシャドウカラーを適用
            right_eye_shadow_colored_fill = np.zeros_like(image, dtype=np.uint8)
            cv2.fillPoly(right_eye_shadow_colored_fill, [right_eye_shadow_points], eye_shadow_color)

            # 右目のアイシャドウを元画像とブレンド
            right_annotated_image = (image.astype(np.float32) * (1 - right_eye_shadow_alpha[:, :, None]) + 
                                    right_eye_shadow_colored_fill.astype(np.float32) * right_eye_shadow_alpha[:, :, None]).astype(np.uint8)

            # 左目と右目のアイシャドウを合成
            annotated_image = cv2.addWeighted(left_annotated_image, 0.5, right_annotated_image, 0.5, 0)

            # 画像が明るくなりすぎないように調整
            annotated_image = np.clip(annotated_image, 0, 255).astype(np.uint8)

            #####アイライナー（右）#####
            for i, num in enumerate(right_eye_upper_line):
                x = int(face_landmarks.landmark[num].x * image.shape[1])
                y = int(face_landmarks.landmark[num].y * image.shape[0])
                # print(f"right eye upper x{x}, y{y} num{num}")
                if i < len(right_eye_upper_line) - 1:
                    # Get the next point's coordinates
                    next_x = int(face_landmarks.landmark[right_eye_upper_line[i + 1]].x * image.shape[1])
                    next_y = int(face_landmarks.landmark[right_eye_upper_line[i + 1]].y * image.shape[0])

                    # Draw a line between the current point and the next point
                    cv2.line(annotated_image, (x, y), (next_x, next_y), eye_line_color, eye_line_thickness, lineType=cv2.LINE_AA)

            #####アイライナー（左）#####
            for i, num in enumerate(left_eye_upper_line):
                x = int(face_landmarks.landmark[num].x * image.shape[1])
                y = int(face_landmarks.landmark[num].y * image.shape[0])
                # print(f"right eye upper x{x}, y{y} num{num}")
                if i < len(left_eye_upper_line) - 1:
                    # Get the next point's coordinates
                    next_x = int(face_landmarks.landmark[left_eye_upper_line[i + 1]].x * image.shape[1])
                    next_y = int(face_landmarks.landmark[left_eye_upper_line[i + 1]].y * image.shape[0])

                    # Draw a line between the current point and the next point
                    cv2.line(annotated_image, (x, y), (next_x, next_y), eye_line_color, eye_line_thickness, lineType=cv2.LINE_AA)

            # for i, num in enumerate(sorted_filtered_face_numbers):
            #     x = int(face_landmarks.landmark[num].x * image.shape[1])
            #     y = int(face_landmarks.landmark[num].y * image.shape[0])
            #     cv2.circle(annotated_image, (x, y), 1, (255, 255, 255), -1)
                # print(f"i{i} x{x} y{y} num {num}")
                # if i == 20: #y76より上は下のため排除できる
                # #    print(f"i{i} x{x} y{y} num {num}")
                #    cv2.circle(annotated_image, (x, y), 1, (0, 0, 255), -1)
                # if i == 2: #y64より下は上のため排除できる
                # #    print(f"i{i} x{x} y{y} num {num}")
            ...
            cv2.fillPoly(annotated_image, [lip_outer_points], rip_color, lineType=cv2.LINE_AA) 

            lip_inner_points = np.array(lip_inner_points, np.int32)
            cv2.fillPoly(annotated_image, [lip_inner_points], rip_color, lineType=cv2.LINE_AA) 

            alpha = 0.5  
            annotated_image = cv2.addWeighted(annotated_image, alpha, image, 1 - alpha, 0)

            # 左目と右目のランドマークを取得
            left_eye = [face_landmarks.landmark[i] for i in [33, 133]]  # 左目端
            right_eye = [face_landmarks.landmark[i] for i in [362, 263]]  # 右目端

            # 左右の目の中心座標を計算
            left_eye_center = (
                int((left_eye[0].x + left_eye[1].x) / 2 * image.shape[1]),
                int((left_eye[0].y + left_eye[1].y) / 2 * image.shape[0]),
            )
            right_eye_center = (
                int((right_eye[0].x + right_eye[1].x) / 2 * image.shape[1]),
                int((right_eye[0].y + right_eye[1].y) / 2 * image.shape[0]),
            )

            # サングラスを配置する幅と高さを計算
            # *2は目の中心感の距離を2倍にしてサングラスの幅を自然な大きさに調整している
            # np.linalg.normで距離計算
            glasses_width = int(np.linalg.norm(np.array(right_eye_center) - np.array(left_eye_center)) * 2)
            # glasses_widthに応じて、元の画像の高さと幅の比率で高さを調整
            glasses_height = int(glasses_width * sunglasses.shape[0] / sunglasses.shape[1])

            # サングラス画像をリサイズ
            resized_glasses = cv2.resize(sunglasses, (glasses_width, glasses_height), interpolation=cv2.INTER_AREA)

            # サングラスの左上座標を計算
            top_left = (
                left_eye_center[0] - glasses_width // 4,
                left_eye_center[1] - glasses_height // 2,
            )
            # cv2.circle(annotated_image, (left_eye_center[0], left_eye_center[1]), 1, (0, 255, 0), 2)
            # cv2.circle(annotated_image, (top_left[0], top_left[1]), 1, (255, 0, 0), 2)

            # サングラスのRGBAチャンネルを分離
            b, g, r, a = cv2.split(resized_glasses)

            # アルファチャンネルを正規化（0～1の範囲）
            alpha = a / 255.0
            # サングラスを合成

            # サングラスの透明度調整（0～1の範囲、例: 0.8で80%の不透明度）
            overall_alpha = 0.8
            ...
            for y in range(glasses_height):
                for x in range(glasses_width):
                    ...
                    # 背景とサングラスのピクセルを取得
                    bg_pixel = annotated_image[top_left[1] + y, top_left[0] + x]
                    fg_pixel = np.array([b[y, x], g[y, x], r[y, x]])

                    # アルファ値に基づいてブレンド（透明部分を考慮）
                    alpha_value = alpha[y, x] * overall_alpha  # 全体の透明度を適用
                    blended_pixel = alpha_value * fg_pixel + (1 - alpha_value) * bg_pixel
                    # annotated_image[top_left[1] + y, top_left[0] + x] = blended_pixel.astype(np.uint8) #サングラスを外す場合はコメントアウト

        output_image = annotated_image

        # Save the annotated image in the 'outputs' directory
        output_path = os.path.join(output_dir, f"annotated_{os.path.basename(file)}")
        cv2.imwrite(output_path, output_image)
        print(f"Annotated image saved as {output_path}")
      
        #   cv2.imshow('Face Mesh with Lips', output_image)
